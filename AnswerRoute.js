...
router.get('/RecentA', function (req, res) {
    ...
    con.connect(function (err) {
        con.query(`select * from answers LEFT JOIN answerrating ar ON answers.a_id = ar.a_id WHERE answers.q_id = "${q_id}" ORDER by ar.rating desc`, function (err, results) {
            res.send(results)
        })
    })
})
...
router.put('/UpdateA',function(req,res){
    ...
    let q_id = req.body.q_id
    con.connect(function(err){
        con.query(`Update answers SET answer = "${updA}" WHERE a_id = ${a_id} AND q_id = ${q_id}`,function(err,results){
            res.send(results)
        })
    })
})

//post request for answers (still needs work)
